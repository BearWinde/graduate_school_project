...
if __name__ == "__main__":
    ...
    asyncc = True
    paths = ["test_path","train_path","vaild_path"]
    ...
    for path in paths:
        ...
        if asyncc:
            ...
            for i in range(pbar.total):
                duration_secs = librosa.get_duration(path=audio_path_prefix+audio_path[i])
                print(123)
                pool.apply_async(preprocess, 
                                args=(os.path.join(audio_path_prefix, audio_path[i]),
                                    ...
                                    config["common"]["sampling_rate"] *int(duration_secs), 
                                    True,
